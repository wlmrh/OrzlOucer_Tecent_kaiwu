...
class Agent(BaseAgent):
    def __init__(self, agent_type="player", device=None, logger=None, monitor=None) -> None:
        ...
        self.algorithm = Algorithm(self.logger, self.gamma, self.learning_rate, self.state_size, self.action_size)
    ...
    def _epsilon_greedy(self, state, epsilon=0.1):
        ...
        if np.random.rand() <= epsilon:
            ...
            with torch.no_grad():
                q_values = self.algorithm.policy_net(state)
    ...
    def exploit(self, list_obs_data):
        ...
        act_data = self.algorithm.policy_net(state).argmax()
    ...
    def save_model(self, path=None, id="1"):
        ...
        np.save(model_file_path, self.algorithm.policy_net)
    ...
    def load_model(self, path=None, id="1"):
        ...
        model_file_path = f"{path}/model.ckpt-{str(id)}.npy"
        try:
            self.algorithm.policy_net = np.load(model_file_path)
            self.algorithm.target_net.load_state_dict(self.algorithm.policy_net.state_dict())
