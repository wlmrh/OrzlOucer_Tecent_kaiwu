...
class Algorithm:
    def __init__(self, device, logger, monitor):
        self.act_shape = Config.DIM_OF_TOTAL_ACTION  # 总动作空间维度
    ...
    def predict_detail(self, list_obs_data, exploit_flag=False):
        ...
        format_action = []
        for instance in act:
            action_id = instance[0]
            if action_id < self.direction_space:
                # 普通移动动作 (0-7)
                format_action.append([action_id, 0])
            else:
                # 超级闪现动作 (8-15)
                talent_dir = action_id - self.direction_space
                format_action.append([talent_dir, 1])
